...
public final class CuffedRestrictionListener implements Listener {
    private final StreetLifeRPContext ctx;
    private final Map<UUID, Long> lastWarnMillis = new ConcurrentHashMap<>();

    public CuffedRestrictionListener(StreetLifeRPContext ctx) {
        this.ctx = ctx;
    }
    ...
    public void onDamage(EntityDamageByEntityEvent event) {
        ...
        if (!ctx.justice().isCuffed(damager.getUniqueId())) return;
    ...
    private void warn(Player player) {
        ...
        player.sendMessage(ctx.config().prefix() + ChatColor.RED + "Menotté: impossible d'utiliser des objets.");
    }
}
